...
exports.updateProductRating = async (req, res) => {
	...
	if (existingRating === undefined) {
	...
	} else if (star >= existingRating.star) {
		//Find product and update the rating
		// if (star < existingRating.star) {
		// 	res.json({ message: `Your rating is lower than ${existingRating.star}` }).status(400);
		// 	return;
		// }
		...
		console.log(`Current Rating ${existingRating.star}`);
		res.json(updatedRating);
	} else {
		console.log(`Rating ${star}`);
		res.status(400).json({ message: `Your rating is lower than ${existingRating.star} star` });
	}
};
